<template>
    <div class="step1">
        <!-- 心系证候类型 -->
        <div class="type">
            <BaseZyTempTitle bgClass="jgy_bg_5" :tempTitle="'运动计划偏好'"/>
            <div class="ctn">
                <LayBg/>
                <Cell
...
<script>
    ...
    import { apiGetSportEvalutionList,apiSaveCourse } from '@/service/api/sport'
    export default {
        data() {
            return {
                quesOptions: [],
        ...
        computed: {
            ...mapState(['userInfo','code','courseKind']),
            ...mapState('configure', ['sportWayCode']),
            saveFlag() {
        ...
        methods: {
            ...mapMutations('configure', ['setUserQuesOptionsAnser','setSportEvaluationId','setWeeklyExerciseFrequencies']),
            async getEvaluationList(){
                const data  = await apiGetSportEvalutionList(this.code, window.sessionStorage.getItem('resultCode'),this.userInfo.userId,this.sportWayCode)
                this.quesOptions = data.quesOptions
                this.sportEvaluationId = data.evaluationId
            },
            cellClick(index) {
            ...
            async saveCourse () {
                ...
                const data = await apiSaveCourse(this.code,window.sessionStorage.getItem("resultCode"), this.userInfo.userId, userOptions, this.sportEvaluationId)
                this.setWeeklyExerciseFrequencies(data.weeklyExerciseFrequencies)
                this.setSportEvaluationId(data.evaluationId)
             
                this.$router.push('/steptrainingday')
            },
            // 保存按钮 
            saveClick() {
                if (this.saveFlag) {
